...
class InMemoryDeduplicator:
    ...
    def _build_key(self, node_id, input_queue, message):
        return DedupKey(
            node_id=str(node_id),
    ...
    def should_process(self, node_id, input_queue, message):
        if message.message_id is None:
            return True

        key = self._build_key(node_id, input_queue, message)
        with self._lock:
    ...
    def mark_processed(self, node_id, input_queue, message):
        if message.message_id is None:
            return
    ...
    def process_once(self, node_id, input_queue, message, process):
        if message.message_id is None:
            process()
            return True
        ...
        with self._lock:
            ...
            if ranges.contains(message.message_id):
                logging.info(
                    "Discarding duplicate message. node_id=%s input_queue=%s client=%s "
                    "type=%s message_id=%s",
                    node_id,
                    input_queue,
