...
class BankFilter:
    ...
    def _handle_eof_message(self, cid, data):
        logging.info(f"BankFilter {self.id} recibió mensaje EOF para el cliente {cid}")
    ...
    def _on_send_eof_to_joiner(self, client_id, totals_by_output, origin_worker_prefix, amount_origin_workers):
        ...
        logging.info(f"BankFilter {self.id} (Líder) envió EOF final al Joiner para cliente {client_id}")
    ...
    def _clean_client_memory(self, client_id):
        ...
        if client_id in self.seen_banks:
            ...
            logging.info(f"Memoria de bancos limpiada para el cliente {client_id} en BankFilter {self.id}")
